...
import { getNextCard, getTotalActiveCards, findCardByHanzi } from './dictionary.js';
...
// ─── Static file serving ────────────────────────────────────────────────────
const VOCAB_ROOT = path.resolve(__dirname, '../opencodeversion');
...
app.get('/api/next-card', (_req, res) => {
  try {
    ...
    if (card.sentenceText) {
      ...
      for (const ch of [...card.sentenceText]) {
        if (/[\u4e00-\u9fff]/.test(ch) && !sentencePinyinMap[ch]) {
...
const stmtFocusedShengmu = db.prepare(`
  ...
  GROUP BY word_id HAVING total_attempts > 0
