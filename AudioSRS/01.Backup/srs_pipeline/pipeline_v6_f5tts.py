            ...
            for attempt in range(2):
                try:
                    audio_data, sample_rate, _ = model.infer(
                        ...
                        ref_text=ref_text,
                        gen_text=final_text,
                        nfe_step=CFG.get("f5_nfe_step", 16) if attempt == 0 else max(8, CFG.get("f5_nfe_step", 16) // 2), # Fallback to faster/lighter inference on OOM
                        cfg_strength=CFG.get("f5_cfg_strength", 2.0),
                        sway_sampling_coef=CFG.get("f5_sway_sampling_coef", -1.0),
                        speed=CFG.get("f5_speed", 1.0)
                    )
                    break
