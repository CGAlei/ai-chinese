...
function renderSentence(text) {
  ...
  chars.forEach((ch, i) => {
    if (/[\s，。！？、：；""''「」【】()（）…—\-]/.test(ch)) {
      ...
      rt.textContent = currentCard?.sentencePinyinMap?.[ch] ?? '';
...
function processAzureResult(result) {
  ...
  try {
    ...
    for (const word of (azureRaw?.NBest?.[0]?.Words || [])) {
      ...
      if (word.PronunciationAssessment?.Feedback?.Prosody?.Tone?.ErrorTypes?.length > 0) {
        toneError = true;
      }
    }
  } catch (e) {
  ...
  showResultPanel(overallScore, initialError, finalError, toneError);
...
function processMockResult() {
  ...
  showResultPanel(score, Math.random() > 0.7, Math.random() > 0.6, Math.random() > 0.5);
...
function applyWordColors(words, overall, isMock = false) {
  ...
  rubies.forEach((r) => {
    r.classList.remove('word-pass', 'word-fail', 'word-omit', 'show-pinyin');
  });
  ...
  for (const wr of wordResults) {
    ...
    for (const ch of chars) {
      ...
      if (wr.score >= 0.50) {
        ruby.classList.add('word-pass');
      } else {
        // Mark fail + show its pinyin immediately (rt already pre-populated)
        ruby.classList.add('word-fail', 'show-pinyin');
      }
      rubyIdx++;
    }
  }
  ...
  rubyList.forEach((ruby) => {
    if (!ruby.classList.contains('word-pass') && !ruby.classList.contains('word-fail')) {
 ...
 */
function showResultPanel(score, initialError, finalError, toneError) {
  ...
  if (pct >= 75) {
    scoreBar.style.background = 'linear-gradient(90deg, #059669, #34d399)';
  } else if (pct >= 50) {
  ...
  if (pct >= 90) {
    ...
    resultBadge.className   = 'text-xs font-bold px-3 py-1 rounded-full bg-brand-500/20 text-brand-300';
  } else if (pct >= 50) {
    resultBadge.textContent = '加油 💪';
    resultBadge.className   = 'text-xs font-bold px-3 py-1 rounded-full bg-amber-500/20 text-amber-300';
  } else {
    resultBadge.textContent = '再试 🔄';
    resultBadge.className   = 'text-xs font-bold px-3 py-1 rounded-full bg-red-500/20 text-red-300';
  }
