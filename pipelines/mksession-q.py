...
def run_local_qwen_transcription(audio_path: str, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    # Setup appropriate precision and check VRAM
    if device == "cuda":
        try:
            free_mem, total_mem = torch.cuda.mem_get_info()
            logger.info(f"     -> GPU VRAM Status: Free: {free_mem / 1024**3:.2f} GB | Total: {total_mem / 1024**3:.2f} GB")
            # If free memory is less than 1.5 GB, fall back to CPU for ASR
            if free_mem < 1.5 * 1024 * 1024 * 1024:
                logger.warning(f"Very low free VRAM ({free_mem / 1024**3:.2f} GB). Falling back to CPU for transcription to prevent system crash.")
                device = "cpu"
        except Exception as e:
            logger.warning(f"Could not retrieve GPU VRAM info: {e}")
    ...
    if device == "cuda":
        try:
            free_mem, total_mem = torch.cuda.mem_get_info()
            logger.info(f"     -> GPU VRAM Status before Aligner: Free: {free_mem / 1024**3:.2f} GB")
            # If free VRAM is less than 2.0 GB, load aligner on CPU to prevent driver lockup/crash
            if free_mem < 2.0 * 1024 * 1024 * 1024:
                logger.warning(f"Low free VRAM ({free_mem / 1024**3:.2f} GB). Loading Forced Aligner on CPU to prevent lockup.")
                aligner_device = "cpu"
                aligner_dtype = torch.float32
        except Exception as e:
            logger.warning(f"Could not retrieve GPU VRAM info: {e}")

    logger.info(f"     -> Aligner Model: {aligner_model_name} | Device: {aligner_device}")

    try:
        aligner = Qwen3ForcedAligner.from_pretrained(
            aligner_model_name,
            dtype=aligner_dtype,
            device_map=aligner_device,
        )
